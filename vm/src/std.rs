...
pub mod types {
    ...
    pub fn load_libs(libs: &mut HashMap<String, Value>) {
        libs.insert("types".to_owned(), Value::new_lib_obj(|obj| {
            obj.insert("type_name".to_owned(), Value::NativeFunction(|args| {
                if args.len() != 1 {
                    return Err(VMError::IllegalFunctionArguments);
                }
                Ok(Value::new_str(args[0].type_to_str()))
            }));
            obj.insert("to_string".to_owned(), Value::NativeFunction(|args| {
                if args.len() != 1 {
                    return Err(VMError::IllegalFunctionArguments);
                }
                Ok(Value::new_str(args[0].to_string()))
            }));
            obj.insert("int_to_float".to_owned(), Value::NativeFunction(|args| {
                if args.len() != 1 {
                    return Err(VMError::IllegalFunctionArguments);
                }
                let i = args[0].as_int()
            ...
            obj.insert("float_to_int".to_owned(), Value::NativeFunction(|args| {
                if args.len() != 1 {
                    return Err(VMError::IllegalFunctionArguments);
                }
            ...
            obj.insert("string_to_int".to_owned(), Value::NativeFunction(|args| {
                if args.len() != 1 {
                    return Err(VMError::IllegalFunctionArguments);
                }
                let s = args[0].as_str()
            ...
            obj.insert("string_to_float".to_owned(), Value::NativeFunction(|args| {
                if args.len() != 1 {
                    return Err(VMError::IllegalFunctionArguments);
                }
                let s = args[0].as_str()
