...
pub fn load_libs(ctx: &mut Context) {
    ...
    lib.insert("type_name".into(), Value::NativeFunction(|_, args| {
    ...
    lib.insert("int_to_float".into(), Value::NativeFunction(|_, args| {
        Value::check_arg_cnt(&args, 1)?;
        let i = args[0].as_int()?;
        Ok(Value::Float(i as f64))
    }));

    lib.insert("float_to_int".into(), Value::NativeFunction(|_, args| {
        Value::check_arg_cnt(&args, 1)?;
        let f = args[0].as_float()?;
        Ok(Value::Int(f as i64))
    }));

    lib.insert("string_to_int".into(), Value::NativeFunction(|_, args| {
        Value::check_arg_cnt(&args, 1)?;
        let s = args[0].as_str()?;
    ...
    lib.insert("string_to_float".into(), Value::NativeFunction(|_, args| {
        Value::check_arg_cnt(&args, 1)?;
        let s = args[0].as_str()?;
